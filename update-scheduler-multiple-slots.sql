-- ACTUALIZAR FUNCIÓN SCHEDULER PARA MÚLTIPLES SLOTS
...
BEGIN
    ...
    -- Insertar registros automáticos (ahora permite múltiples slots por día)
    INSERT INTO work_logs (
        user_id, date, start_time, end_time, total_hours, type, is_auto_generated, slot_id, created_at, updated_at
    )
    SELECT DISTINCT
        ats.user_id,
        spain_date,
        ...
        'work',
        true,
        ats.slot_id, -- Usar slot_id de la configuración
        NOW(),
        NOW()
    ...
    -- Solo si no existe registro para hoy con el mismo slot_id
    AND NOT EXISTS (
        SELECT 1 FROM work_logs wl
        WHERE wl.user_id = ats.user_id
        AND wl.date = spain_date
        AND wl.slot_id = ats.slot_id
    );
